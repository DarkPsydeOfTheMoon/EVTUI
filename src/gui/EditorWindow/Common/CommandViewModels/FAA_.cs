...
public class FAA_ : Generic
{
    public FAA_(DataManager config, CommonViewModels commonVMs, CommandPointer cmd) : base(config, commonVMs, cmd)
    {
        ...
        this.FirstAnimation = new AnimationWidget(config, commonVMs, this.AssetID, this.CommandData.FirstAnimation, this.CommandData.Flags, $"First Animation", frameBlendingInd:1, trackNum:0);
        this.SecondAnimation = new AnimationWidget(config, commonVMs, this.AssetID, this.CommandData.SecondAnimation, this.CommandData.Flags, $"Second Animation", enabledInd:0, frameBlendingInd:2, enabledFlip:true, trackNum:0);
    }
